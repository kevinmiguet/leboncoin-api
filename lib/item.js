    ...
    getDetails() {
        ...
        return new Promise(function (resolve, reject) {
            request.get({
                ...
                const scriptStart = body.indexOf('window.FLUX_STATE = ');
                const scriptEnd = body.indexOf('</script>', scriptStart);
                const details = JSON.parse(body.substring(scriptStart, scriptEnd).replace("window.FLUX_STATE = ", ""));
                const opts = {
...
const parseTableData = function ($) {
    ...
    $('.properties > .line').each(function (index, row) {
        const key = $(row).find('.property').text().toLowerCase().replace(":", "").trim();
        const value = cleanString($(row).find('.value').text());
        if(key && value) {
